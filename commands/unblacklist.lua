...
return {
    name = 'unblacklist',
    description = 'Unblacklists a user for your paste or wtv',
    options = {
        {
            name = 'user',
    ...
    run = function(interaction)
        ...
        local suc, res = discordia.extra.git:unblacklist(targetUser)
        if res then
            return interaction:reply({
                content = res
            }, true)
        ...
        return interaction:reply({
            content = 'Unblacklisted <@'..targetUser..'>! They will no longer be blacklisted.'
        }, false)
    end
}